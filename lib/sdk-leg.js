...
export class SdkLeg {
  ...
  async prepare() {
    ...
    this._readyPromise = (async () => {
      ...
      // 模板自动落位：数据目录缺官方模板时从插件 bundled/ 复制（只复制 cordis.yml + package.json
      // 两个模板文件，node_modules 一律由官方安装命令产出；这是「按需引导」的自动部分）
      try {
        const pluginBundled = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', 'bundled');
        const srcCordis = path.join(pluginBundled, 'cordis.yml');
        if (fs.existsSync(srcCordis) && !fs.existsSync(cordis)) {
          fs.mkdirSync(bundledDir, { recursive: true });
          fs.copyFileSync(srcCordis, cordis);
          const srcPkg = path.join(pluginBundled, 'package.json');
          if (fs.existsSync(srcPkg)) fs.copyFileSync(srcPkg, path.join(bundledDir, 'package.json'));
        }
      } catch {
        // 落位失败静默：下面存在性检查会给同样的人话引导
      }
  ...
  async _newHarness({ cwd, timeoutMs, envOverrides }) {
    ...
    // 端点可配：留空走官方默认（api.deepseek.com）；填了就指向 OpenAI 兼容网关
    if (typeof this.config?.baseUrl === 'string' && this.config.baseUrl.trim()) {
      env.DSH_LLM_BASE_URL = this.config.baseUrl.trim();
    }
  ...
  async run({ task, cwd, tag, timeoutMs, env, signal, onProgress, opId } = {}) {
    ...
    const onNotification = (n) => {
      ...
      if (n.method === 'session.status') {
        return; // idle/running 转换由 run 返回兜底，不引入中间伪状态
      }
      ...
      if (ev.type === 'assistant/chunk') {
        ...
        // usage 兜底收集（assistant/message 也可能带 usage，task.js 同款链）
        const usage = ev.usage ?? ev.data?.usage ?? msg?.usage;
    ...
    /** 失败原因提取：turn/end 的 reason / failure 里能捞到的都捞，捞不到也给一句人话（失败不无声）*/
    const failReason = (ev) => {
      const reason = ev?.reason ?? ev?.data?.reason ?? {};
      const msg =
        reason?.message ??
      ...
      const head = kind ? `内置 runtime 回合失败：kind=${kind}` : '内置 runtime 回合失败';
      return (detail && detail !== '{}' ? `${head}；reason=${detail}` : `${head}（未附原因）`).slice(0, 2000);
    };
    ...
    const judgeStop = (events) => {
      for (let i = events.length - 1; i >= 0; i--) {
        ...
        if (ev?.type === 'turn/end') {
          ...
          return { ok: false, status: 'error', stopReason: 'error', error: failReason(ev) };
        }
      }
      // 无 turn/end（断流兜底）
    ...
    // ---- 外部取消（宿主中断）：abort → 标记取消并 close runtime（官方放弃语义） ----
    ...
    const onAbort = () => {
      ...
      abortReject(Object.assign(new Error('dsh_run 已取消'), { code: 'DSH_ABORTED' }));
    };
    if (signal) {
    ...
    let harness = null;
    // 总执行预算（dsh_run timeout 契约）：SDK 的 requestTimeoutMs 只限单个 wire
    // 请求，任务总时长必须由本侧计时；到期放弃 runtime（官方语义），终态 timeout。
    ...
    try {
      ...
      const r = await Promise.race([harness.run(text, { onNotification }), abortPromise]);
      ...
      const isTimeout =
        entry.timedOut === true || (/timeout|超时/i.test(msg) && !/shutdown/i.test(msg));
      const terminal = buildTerminal(
        false,
        'error',
      ...
      // 终态回收进程（官方 close 阶梯；失败不阻塞终态；abort 后 close 让 run reject 归入 aborted 分支）
      if (harness) {
        try {
          await harness.close();
        } catch {
