...
export class TaskRunner {
  ...
  async run({ task, cwd, tag, sessionId, agentPreset, timeoutMs, signal, onProgress, opId } = {}) {
    ...
    try {
      ...
      // 6. 清理（协议实测 4.4 finally）：清审批计时器、移除运行记录、解绑外部 signal
  ...
  _onApprovalResolved(ctx, frame) {
    ...
    // 任务记录审批历史（P0-1）：超时自动拒绝标记 rejected(auto) 保留，不被解决帧覆盖
      ...
      this._respond(pending, 'rejected')
        .then(() => {
          if (pending.status === 'pending') {
            ...
            // 任务记录审批历史（P0-1）：自动拒绝有据可查，outcome 记为 rejected(auto)
            this._recordApproval(ctx, {
  ...
  async respondApproval(approvalId, outcome = 'allowed-once') {
    ...
    // 任务记录审批历史（P0-1）：用户应答立即落账（不依赖随后到达的 resolved 帧）
