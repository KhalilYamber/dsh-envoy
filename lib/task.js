...
export class TaskRunner {
   ...
   * @param {string} [opts.mode]               'external' | 'embedded'
   * @param {string} [opts.externalModel]      外接模式派单模型 id（留空 = 不干预，跟随 DSH 默认）
   * @param {string} [opts.externalReasoningEffort] 外接模式派单思考强度（'default' 或留空 = 不干预）
   * @param {string} [opts.externalWorkspace]  外接偏好工作区名（留空 = 不特殊寻找，落 DSH 默认）
   */
  constructor({
    client,
    labels,
    ...
    // 外接模式派单模型/强度（插件设置 externalModel / externalReasoningEffort）：
    // 留空 = 不干预，跟随 DSH 部署默认（session/modelCatalog 的 default）
    this.externalModel = externalModel ?? '';
    this.externalReasoningEffort = externalReasoningEffort ?? '';
  ...
  async run({ task, cwd, tag, sessionId, agentPreset, timeoutMs, signal, onProgress, opId } = {}) {
    ...
    const consume = async () => {
      for await (const frame of sessionFeed(this.client, sid, ctx.controller.signal)) {
        ...
        if (frame.type === 'snapshot') {
          // 0.1.5：snapshot 回放的是会话历史（seed），session/end-seed 是它的结尾标记。
          // 历史里的 turn/end 与 assistant/message 属于上一次任务，必须先跳过；
          // 否则复用会话时本次回执会取到上一轮的结论（0912 实测：1 秒即误判完成）。
          const records = frame.records ?? [];
          // 取最后一个 end-seed：复用会话可能累积多段 seed，只有最新边界之后才属于本次任务（复核 #2）
          const boundary = records.findLastIndex(
            (rec) => rec && rec.type === 'event' && rec.event && rec.event.type === 'session/end-seed'
          );
          for (let i = boundary >= 0 ? boundary + 1 : 0; i < records.length; i += 1) {
            const rec = records[i];
    ...
    try {
      ...
      // 1. 会话归属与建立（任务书流程 1/2）：
      //    external 未指定目录时归入偏好工作区（插件设置 externalWorkspace）；找不到或 bundled 用 cwd；resume 传 sessionId 不传 cwd
      ...
      // 1.5 外接模式：按插件设置应用派单模型/强度（session/selectModel，作用于后续提问）
      try {
        await this._applyModelSelection(sid);
      } catch (e) {
        // 会话已建立但任务未发出：best-effort 取消，避免留下空会话（复核 #8）
        try {
  ...
  async _ensureSession({ sessionId, cwd, agentPreset }) {
    ...
    // 偏好工作区名由插件设置提供（externalWorkspace）；留空 = 不特殊寻找，落 DSH 默认工作区
    const preferWorkspace = String(this.externalWorkspace ?? '').trim();
    if (this.mode === 'external' && !cwd && preferWorkspace) {
      // external 且未指定目录：归入偏好工作区（listWorkspaces 找标题匹配则用其 workspaceId；
      // 找不到不乱建工作区，退到 DSH 默认）。指定了目录时优先用目录建会话（0912 起：便于同目录多工程归位）
      try {
        ...
        const hit = list.find((w) => {
          ...
          return typeof title === 'string' && title.includes(preferWorkspace);
        });
        const wid = hit && (hit.workspaceId ?? hit.id);
        if (wid) payload.workspaceId = wid; // 逐字段赋值：勿整体覆盖，避免挤掉上面的 agentPreset（复核 #1）
      } catch (e) {
        this._log('warn', `[dsh-bridge] 列工作区失败，回退 cwd 建会话：${e?.message || e}`);
      }
    }
    if (!payload.workspaceId && cwd) payload.cwd = cwd; // 同理：逐字段赋值
    const res = await this.client.createSession(payload);
    return this._sessionIdFrom(res, null);
  }

  /**
   * 外接模式派单前应用模型/强度（插件设置 externalModel / externalReasoningEffort）。
   * 协议：session/modelCatalog 取部署默认 provider，session/selectModel 落定本会话选择。
   * 两值都留空时不调用（不干预 DSH 默认）；只填其一时另一项沿用部署默认。
   * @param {string} sessionId 目标会话
   * @returns {Promise<object|null>} 落定的选择（未应用时为 null）
   */
  async _applyModelSelection(sessionId) {
    if (this.mode !== 'external') return null;
    const wantModel = String(this.externalModel ?? '').trim();
    const rawEffort = String(this.externalReasoningEffort ?? '').trim();
    const wantEffort = rawEffort && rawEffort !== 'default' ? rawEffort : '';
    if (!wantModel && !wantEffort) return null;

    // 用户显式填了值就必须生效：读不到目录即明确报错，不做「假降级」（复核 #3）
    let base = null;
    try {
      const catalog = await this.client.modelCatalog();
      base = catalog?.default ?? null;
    } catch (e) {
      throw new Error(
        `外接模式派单模型设置无法生效：读取模型目录失败（session/modelCatalog）：${e?.message || e}`
      );
    }
    const provider = base?.provider ?? null;
    const model = wantModel || base?.model || null;
    if (!provider || !model) {
      throw new Error(
        '外接模式派单模型设置无法生效：未能确定 provider/model。请检查 DSH 的默认模型是否已配置，或清空插件里的「派单模型」设置。'
      );
    }
    const selection = {
      sessionId,
      provider,
      model,
      ...(wantEffort ? { reasoningEffort: wantEffort } : {}),
    };
    try {
      const res = await this.client.selectModel(selection);
      const selected = res?.selected ?? null;
      this._log(
        'info',
        `[dsh-bridge] 派单模型已应用：${selected?.provider ?? provider}/${selected?.model ?? model}` +
          `${selected?.reasoningEffort ? ' @' + selected.reasoningEffort : ''}`
      );
      return selected ?? selection;
    } catch (e) {
      throw new Error(
        `外接模式派单模型设置被 DSH 拒绝（session/selectModel）：${e?.message || e}。请检查模型 id 与思考强度是否被该模型支持。`
      );
    }
  }
